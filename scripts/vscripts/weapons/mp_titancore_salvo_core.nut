...
var function OnWeaponPrimaryAttack_titanweapon_salvocore_rockets( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	foreach ( v in verticalOffsets )
	{
		...
		fireMissileParams.speed = RandomFloatRange( 0.8, 1.1 )
		fireMissileParams.scriptTouchDamageType = damageTypes.projectileImpact
		fireMissileParams.scriptExplosionDamageType = DF_DOOM_FATALITY
...
entity function CreateGuidanceInfoTarget( entity weapon )
{
	...
	#elseif CLIENT
		info_target = CreateClientsideScriptMover( $"mdl/dev/empty_model.rmdl", Vector( 0.0, 0.0, 0.0 ), Vector( 0.0, 0.0, 0.0 ) )
	#endif
